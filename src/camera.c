...
/* Plain data container for adjustment values.
 * GtkAdjustment is opaque in GTK 3, so we can't use it as a stack variable.
 * These structs hold intermediate values before applying to real adjustments
 * via gtk_adjustment_configure(). */
typedef struct _AdjValues AdjValues;
struct _AdjValues {
	double lower;
	double upper;
	double value;
...
static AdjValues prev_x_scrollbar_adj;
static AdjValues prev_y_scrollbar_adj;
...
static void
null_get_scrollbar_states( AdjValues *x_adj, AdjValues *y_adj )
{
	x_adj->lower = 0.0;
	x_adj->upper = 100.0;
	x_adj->value = 0.0;
...
static void
mapv_get_scrollbar_states( AdjValues *x_adj, AdjValues *y_adj )
{
	GNode *dnode;
	XYvec dims, margin;
	XYvec c0, c1;
...
static void
treev_get_scrollbar_states( AdjValues *x_adj, AdjValues *y_adj )
{
	GNode *dnode;
...
/* Reads a real GtkAdjustment into an AdjValues struct */
static void
adj_read( AdjValues *vals, GtkAdjustment *adj )
{
	vals->lower = gtk_adjustment_get_lower( adj );
	vals->upper = gtk_adjustment_get_upper( adj );
	vals->value = gtk_adjustment_get_value( adj );
	vals->step_increment = gtk_adjustment_get_step_increment( adj );
	vals->page_increment = gtk_adjustment_get_page_increment( adj );
	vals->page_size = gtk_adjustment_get_page_size( adj );
}


/* This sets the values of result to be somewhere between the corresponding
 * values of a and b, specified by the interpolation factor k
 * (i.e. if k == 0, then result = a; if k == 1, then result = b, etc.
 * k should be between 0 and 1 inclusive, of course) */
static void
adj_interpolate( AdjValues *result, double k, const AdjValues *a, const AdjValues *b )
{
	result->lower = a->lower + k * (b->lower - a->lower);
	result->upper = a->upper + k * (b->upper - a->upper);
	result->value = a->value + k * (b->value - a->value);
	result->step_increment = a->step_increment + k * (b->step_increment - a->step_increment);
	result->page_increment = a->page_increment + k * (b->page_increment - a->page_increment);
	result->page_size = a->page_size + k * (b->page_size - a->page_size);
}
...
{
	AdjValues x_adj, y_adj;
	AdjValues x_result, y_result;
	...
	/* Block scrollbar callbacks during adjustment updates.
	 * In GTK 3, gtk_adjustment_configure() emits "value-changed"
	 * immediately, which would re-enter camera_scrollbar_move_cb
	 * and overwrite the camera state we just computed. */
	...
	if (camera_moving( )) {
		...
		adj_interpolate( &x_result, camera->pan_part, &prev_x_scrollbar_adj, &x_adj );
		adj_interpolate( &y_result, camera->pan_part, &prev_y_scrollbar_adj, &y_adj );
		adj_apply( x_scrollbar_adj, &x_result );
		adj_apply( y_scrollbar_adj, &y_result );
	}
	else {
		/* Use scrollbar states as-is */
		adj_apply( x_scrollbar_adj, &x_adj );
		adj_apply( y_scrollbar_adj, &y_adj );
	}
...
camera_look_at_full( GNode *node, MorphType mtype, double pan_time_override )
{
	...
	adj_read( &prev_x_scrollbar_adj, x_scrollbar_adj );
	adj_read( &prev_y_scrollbar_adj, y_scrollbar_adj );
 ...
 * In DiscV mode, pans the target point instead (2D top-down view). */
void
camera_revolve( double dtheta, double dphi )
{
	if (globals.fsv_mode == FSV_DISCV) {
		/* DiscV is a top-down 2D view: pan the target point */
		double scale = camera->distance * tan( RAD(0.5 * camera->fov) ) / 180.0;
		DISCV_CAMERA(camera)->target.x += dtheta * scale;
		DISCV_CAMERA(camera)->target.y -= dphi * scale;
	}
	else {
